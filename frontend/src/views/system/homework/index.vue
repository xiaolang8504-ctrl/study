<template>
  <div class="app-container home-work-page">
    <el-form :inline="true" :model="queryParams" size="small" class="filter-form">
      ...
      <el-form-item>
        ...
        <el-button v-if="hasMenuCode(menuCode.HOME_WORK.CREATE)" type="success" icon="el-icon-plus" @click="handleCreate">新增作业</el-button>
      </el-form-item>
    </el-form>

    <el-table v-loading="loading" :data="tableData" border>
      <el-table-column prop="id" label="ID" width="80" />
      <el-table-column label="作业附件" width="150" align="center">
      ...
      <el-table-column v-if="hasHomeWorkRowAction" label="操作" fixed="right" width="210">
        <template slot-scope="{ row }">
          <el-button v-if="hasMenuCode(menuCode.HOME_WORK.DETAIL)" type="text" icon="el-icon-view" @click="handleDetail(row)">详情</el-button>
          <el-button v-if="hasMenuCode(menuCode.HOME_WORK.UPDATE)" type="text" icon="el-icon-edit" @click="handleUpdate(row)">编辑</el-button>
          <el-button v-if="hasMenuCode(menuCode.HOME_WORK.DELETE)" type="text" icon="el-icon-delete" class="danger-action" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="作业详情" :visible.sync="detailOpen" width="660px" append-to-body>
      <el-descriptions v-if="detail" :column="2" border>
        ...
        <el-descriptions-item label="作业附件" :span="2">
          ...
          >
            {{ detail.fileName || '查看作业附件' }}{{ detail.fileSize ? `（${detail.fileSize}）` : '' }}
          </el-button>
          <span v-else>-</span>
        </el-descriptions-item>
...
<script>
import { homeWorkDetail, homeWorkPageList, createHomeWork, deleteHomeWork, updateHomeWork } from '@/api/system/homeWork'
...
const HOME_WORK_UPLOAD_TYPE = 'homeWork'
const DICT_TYPES = {
  grade: 'grade',
  subject: 'subject'
}
...
export default {
  ...
  computed: {
    hasHomeWorkRowAction() {
      return [
        this.menuCode.HOME_WORK.DETAIL,
        this.menuCode.HOME_WORK.UPDATE,
        this.menuCode.HOME_WORK.DELETE
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      homeWorkPageList(this.queryParams).then(data => {
    ...
    handleUpdate(row) {
      homeWorkDetail({ id: row.id }).then(data => {
    ...
    uploadAttachment(options) {
      filePolicy({ uploadType: HOME_WORK_UPLOAD_TYPE }).then(policy => {
    ...
    resolveFileUrl(fileId) {
      ...
      return downloadUrl({ fileId: fileIdNumber, uploadType: HOME_WORK_UPLOAD_TYPE }).catch(() => '')
    },
    fillListCover() {
    ...
    submitForm() {
      this.$refs.formRef.validate(valid => {
        ...
        const request = this.form.id ? updateHomeWork : createHomeWork
        const requestData = {
    ...
    handleDelete(row) {
      this.$confirm('确认删除该作业吗？', '提示', { type: 'warning' }).then(() => {
        return deleteHomeWork({ id: row.id })
      }).then(() => {
